...
POSISI : $link";
      ...
      } else if ($update['message']['audio'] == true) {
        file_get_contents($apiURL . "/sendaudio?chat_id=" . $chatID . "&audio=" . urlencode(($update['message']['voice']['file_id'])) . "&parse_mode=HTML");
      } else if ($update['message']['voice'] == true) {
        file_get_contents($apiURL . "/sendvoice?chat_id=" . $chatID . "&audio=" . urlencode(($update['message']['voice']['file_id'])) . "&parse_mode=HTML");
      } else if ($update['message']['photo'] == true) {
        file_get_contents($apiURL . "/sendPhoto?chat_id=" . $chatID . "&photo=" . urlencode(($update['message']['photo']['file_id'])) . "&parse_mode=HTML");
      } else {
        file_get_contents($apiURL . "/sendmessage?chat_id=" . $chatID . "&text=" . urlencode(json_encode($update)) . "&parse_mode=HTML");
      }
    }
  }
